...
class GameState:
    def __init__(
            self, players: list[Player], deck: Deck, top: lamaCard, current_player: int = 0
    ):
        self.players: list[Player] = players
        self.deck: Deck = deck
        self.top: lamaCard = top
    ...
    def draw_card(self) -> lamaCard: #Метод, позволяющий текущему игроку взять карту из колоды
    ...
    def player_action(self): #Метод, который обрабатывает действия текущего игрока, запрашивая у него выбор действия (играть, взять или выйти)
        ...
        while True:
            ...
            if action == "Играть":
                ...
                try:
                    card = lamaCard(int(card_value))  # Пробуем создать карту
